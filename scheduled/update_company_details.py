...
for int_id in comp_list.loc[:,'intrinio_id'].to_list():
    ...
    temp_pd = pd.DataFrame(temp_dict, index = [0]).rename(columns = {'id':'intrinio_id', 'name':'company', 'template':'finstate_template'}).loc[:,comp_dets.columns[~comp_dets.columns.isin(['update_date', 'insert_date', 'is_current'])]]
    # if the company data is not in the database add it
    if len(comp_dets.loc[comp_dets['intrinio_id']==int_id,:]) == 0:
    ...
    elif temp_pd.equals(comp_dets.loc[comp_dets['intrinio_id']==int_id, comp_dets.columns[~comp_dets.columns.isin(['update_date', 'insert_date', 'is_current'])]].reset_index(drop = True)):
        # api pull = values in database; just update the update_date to reflect the last check
        conn.execute("update dbo.company_details set update_date = '"+dt.date.today().strftime("%Y-%m-%d")+"' where intrinio_id = '"+int_id+"' and is_current = 1")
    else:
        # api pull != values in database;
        # set the is_current flag = 0
        conn.execute("update dbo.company_details set update_date = '"+dt.date.today().strftime("%Y-%m-%d")+"' , is_current = 0 where intrinio_id = '"+int_id+"' and is_current = 1")
        # add the api pulled values
        temp_pd.assign(insert_date = comp_dets.loc[comp_dets['intrinio_id']==int_id, 'insert_date']).assign(update_date = dt.date.today()).assign(is_current = 1).to_sql('company_details', schema = 'dbo', con = conn, index = False, if_exists = 'append')
