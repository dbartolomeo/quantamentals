...
err = pd.DataFrame(columns = ['identifier', 'start_date', 'end_date', 'page_size', 'next_page', 'err_msg'])
...
for sec in sec_list.loc[:,'intrinio_id'].to_list():
    ...
    dates_in_table = pd.read_sql(sql = "select asofdate from dbo.security_priceadj where intrinio_id = '"+sec+"'", parse_dates = ['asofdate'],con = conn).loc[:,'asofdate']
    ...
    store = pd.DataFrame(columns = ['intrinio_id', 'asofdate', 'factor', 'dividend', 'dividend_currency', 'split_ratio'])
    # keep on making API calls per security until there are no more pages to pull from
    while next_page is not None:
        try:
            # API CALL
            res = intrinio_sdk.SecurityApi().get_security_stock_price_adjustments(identifier, start_date=start_date, end_date=end_date, page_size=page_size, next_page=next_page).to_dict()
            # get the prices from the call result
            temp = pd.DataFrame(res.get('stock_price_adjustments',[])).assign(intrinio_id = sec).rename(columns = {'date':'asofdate'}).reindex(['intrinio_id', 'asofdate', 'factor', 'dividend', 'dividend_currency', 'split_ratio'], axis = 'columns')
            ...
            err = pd.concat([err, pd.DataFrame([[identifier, start_date, end_date, page_size, next_page, str(err_msg)]], columns = ['identifier', 'start_date', 'end_date', 'page_size', 'next_page', 'err_msg'])], axis = 'index').reset_index(drop = True)
    ...
    if len(store) != 0:
        store.to_sql('security_priceadj', schema = 'dbo', con = conn, index = False, if_exists = 'append')
